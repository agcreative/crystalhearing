 ...
 * Description: Backup, Restore, and Migrate. Backs up all your files, settings, and content allowing rapid restoration or migration to a new host or URL.
 * Version: 1.3.20
...
if (!class_exists("iThemesBackupBuddy")) {
	class iThemesBackupBuddy {
		...
		var $_url = 'http://pluginbuddy.com/purchase/backupbuddy/';
		...
		function iThemesBackupBuddy() {
			...
			if ( is_admin() ) {
				...
				add_action('wp_ajax_backupbuddy_ftptest', array(&$this, 'ajax_ftptest') ); // Test FTP
				add_action('wp_ajax_backupbuddy_awstest', array(&$this, 'ajax_awstest') ); // Test Amazon S3
				add_action('wp_ajax_backupbuddy_dirlist', array(&$this, 'ajax_dirlist') ); // Directory listing for exluding
				
				add_action('wp_dashboard_setup', array( &$this, 'add_dashboard_widgets' ) ); // Dashboard Stats
			...
			<div class="wrap">
				<div class="postbox-container" style="width:70%;">
					...
					<div id="pluginbuddy_debugtoggle_div" style="display: none;">
						...
						if ( file_exists( WP_CONTENT_DIR . '/uploads/backupbuddy_backups/' . $this->_var . '-' . get_option('ithemes-backupbuddy-updater-guid') . '.txt' ) ) {
							echo readfile( WP_CONTENT_DIR . '/uploads/backupbuddy_backups/' . $this->_var . '-' . get_option('ithemes-backupbuddy-updater-guid') . '.txt' );
						} else {
		...
		function view_backup() {
			...
			if (!empty($_GET['backup_step'])) {
				...
				echo 'Having trouble? Check out our <a target="_blank" href="http://ithemes.com/codex/page/BackupBuddy">BackupBuddy Wiki for comprehensive troubleshooting information</a><br />';
				?>
				<h3>Backup Status:</h3>
				<div class="updated">
			...
			?>
				Backups stored in <?php echo str_replace( '\\', '/', $this->_options['backup_directory'] ); ?> <?php $this->tip( 'This is the local directory that backups are stored in. Backup files include random characters in their name for increased security. Verify that write permissions are available for this directory.' ); ?><br />
				<p>
					<ol>
		...
		function cron_schedule($item) {
			...
			if ( is_array($this->_options['schedules'][$item]) ) {
				...
				if ( $got_aws ) {
					...
					wp_schedule_single_event( time(), $this->_var.'-cron_aws', array($this->_options['aws_accesskey'], $this->_options['aws_secretkey'], $this->_options['aws_bucket'], $this->_options['aws_directory'], $this->zip_file, $scheduled['delete_after']) );
					spawn_cron();
					$this->log( 'Scheduled to run Amazon S3 cron for just-completed-full-backup now on next page load.' );
				}
			}
		}
		...
		function _get_storage_directory( $type, $serial, $create_new = false ) {
		...
		function _pre_backup( $type ) {
			...
			$storage_directory = $this->_get_storage_directory( $type, $serial, true );
			...
			$this->log( 'Finished pre-backup.' );
		}
		
		
		function _post_backup( $type ) {
			$this->log( 'Starting post-backup.' );
			
			if ( file_exists( $this->_options['backup_status']['storage_directory'] . '/.htaccess' ) ) {
		...
		function _backup( $type, $cronning = false ) {
			...
			*/
			...
			$this->log( 'Finished backing up.' );
		}
		
		function _run_backup_processes() {
			$this->log( 'Running backup processes. _run_backup_processes().' );
//			$this->_start_time = time();
			...
			$this->log( 'Finished backup processes.' );
		}
		...
		function _create_settings_file( $process ) {
			$this->log( 'Creating settings file.' );
			global $wpdb;
			...
			$this->log( 'Finished settings file.' );
		...
		function _create_database_backup_file( $process ) {
			...
			$this->log( 'Finished database dump.' );
		...
		function _create_database_backup( $process ) {
			...
			*/
			...
			$this->log( 'Finished database backup.' );
		...
		function cron_ftp( $ftp_server, $ftp_user, $ftp_pass, $ftp_path, $file, $ftp_type, $delete_after = false ) {
			...
			if ( $login_result === false ) {
				...
				$this->mail_notice( 'ERROR #9011 ( http://ithemes.com/codex/page/BackupBuddy:_Error_Codes#9011 ).  FTP/FTPs login failed on scheduled FTP. Details: ' . $details );
				die();
			} else {
			...
			if ( $upload === false ) {
				...
				$this->mail_notice( 'ERROR #9012 ( http://ithemes.com/codex/page/BackupBuddy:_Error_Codes#9012 ).  FTP/FTPs file upload failed. Check file permissions & disk quota. Details: ' . $details );
			} else {
		...
		function log( $text, $log_type = 'all' ) {
			...
			if ( $write === true ) {
				// TODO use a different unique ID for lite version compat.
				$fh = fopen( WP_CONTENT_DIR . '/uploads/backupbuddy_backups/' . $this->_var . '-' . get_option('ithemes-backupbuddy-updater-guid') . '.txt', 'a');
